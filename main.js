...
async function getDeviceCode(clientId) {
  ...
  if (!res.ok)
    throw new Error(`Trakt device code error: ${res.status}`);
  ...
  while (true) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        token.expires_at = Date.now() / 1e3 + token.expires_in;
        return token;
...
async function getTraktHistory(accessToken, clientId) {
  let allItems = [];
  let page = 1;
  const perPage = 100;
  while (true) {
    ...
    if (!res.ok)
      throw new Error(`Trakt history error: ${res.status}`);
...
async function getTMDBMovie(apiKey, tmdbId) {
  const res = await axios_default.get(`${TMDB_BASE_URL}/movie/${tmdbId}`, {
    params: { api_key: apiKey }
  });
  return res.data;
}
async function getTMDBShow(apiKey, tmdbId) {
  const res = await axios_default.get(`${TMDB_BASE_URL}/tv/${tmdbId}`, {
    params: { api_key: apiKey }
  });
  return res.data;
}
...
`;
  if (genres !== void 0)
    content += `${propGenres}: [${(genres ?? []).map((g) => `"${g}"`).join(", ")}]
`;
  if (backdrop !== void 0)
  ...
  async syncTraktHistory() {
    try {
      if (!this.settings.traktClientId || !this.settings.traktClientSecret || !this.settings.tmdbApiKey) {
        new import_obsidian.Notice("Please set your Trakt and TMDB API keys in the plugin settings.");
        return;
      }
      let token = await this.loadToken();
      if (!token || token.expires_at && token.expires_at < Date.now() / 1e3) {
      ...
      let progressNotice = new import_obsidian.Notice("Fetching Trakt history...");
      ...
      for (const [key, items] of Object.entries(grouped)) {
        let mediaType = key.startsWith("movie-") ? "movie" : "show";
        let title = "";
        let traktId = "";
        let tmdbId = "";
        let notePath = "";
        ...
        if (mediaType === "movie") {
          ...
          notePath = (0, import_obsidian.normalizePath)(`${this.settings.movieFolder}/${(0, import_slugify.default)(`${String(title)} ${String(movie.year)}`)}.md`);
        } else {
          ...
          notePath = (0, import_obsidian.normalizePath)(`${this.settings.showFolder}/${(0, import_slugify.default)(`${String(title)} ${String(show.year)}`)}.md`);
          ...
          if (tmdbData && tmdbData.seasons && lastEpisodeInfo) {
            ...
            const lastSeason = Math.max(...tmdbData.seasons.filter((s) => s.season_number > 0).map((s) => s.season_number));
            const lastSeasonObj = tmdbData.seasons.find((s) => s.season_number === lastSeason);
        ...
        const markdown = buildMarkdown({
          ...
          lastWatched,
          watchedCount,
          lastEpisodeInfo: this.settings.includeLastEpisodeWatched ? lastEpisodeInfo : void 0,
          traktId: this.settings.includeTraktId ? traktId : void 0,
          tmdbId: this.settings.includeTmdbId ? tmdbId : void 0,
          releaseDate: this.settings.includeReleaseDate ? releaseDate : void 0,
          genres: this.settings.includeGenres ? genres : void 0,
          backdrop: this.settings.includeBackdrop ? backdrop : void 0,
      ...
      new import_obsidian.Notice(`Trakt Sync error: ${e.message || e}`);
      console.error(e);
    }
  }
  async loadToken() {
...
var TraktDeviceCodeModal = class extends import_obsidian.Modal {
  constructor(app, code, url2, expiresIn) {
    super(app);
    this.code = code;
    this.url = url2;
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).addButton(
      (btn) => btn.setButtonText("Copy Code").onClick(() => {
...
var TraktSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  async clearToken() {
    await this.plugin.saveToken({});
    new import_obsidian.Notice("Trakt token cleared.");
    this.display();
  }
  async getTokenStatus() {
    ...
    if (!token || !token.access_token)
      return "No token";
    if (token.expires_at && token.expires_at < Date.now() / 1e3)
      return "Expired";
    return "Valid";
  }
  async display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Movies folder").setDesc("Choose where movie entries should be saved.").addDropdown((dropdown) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Shows folder").setDesc("Choose where show entries should be saved.").addDropdown((dropdown) => {
      folders.forEach((folder) => dropdown.addOption(folder, folder));
    ...
    new import_obsidian.Setting(containerEl).setName("Trakt client ID").addText(
      (text) => text.setPlaceholder("Enter your Trakt client ID").setValue(this.plugin.settings.traktClientId).onChange(async (value) => {
        this.plugin.settings.traktClientId = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Trakt client secret").addText(
      (text) => text.setPlaceholder("Enter your Trakt client secret").setValue(this.plugin.settings.traktClientSecret).onChange(async (value) => {
        this.plugin.settings.traktClientSecret = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("TMDB API key").addText(
    ...
    new import_obsidian.Setting(containerEl).setName("YAML customisation").setHeading();
    new import_obsidian.Setting(containerEl).setName("Tag format").setDesc("Choose how tags are formatted in frontmatter.").addDropdown(
      (dropdown) => dropdown.addOption("plain", "plain (movie)").addOption("hash", "hash (#movie)").setValue(this.plugin.settings.tagFormat).onChange(async (value) => {
        this.plugin.settings.tagFormat = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Release date").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.includeReleaseDate).onChange(async (value) => {
        this.plugin.settings.includeReleaseDate = value;
        await this.plugin.saveSettings();
        this.display();
      })
    ).addText(
      (text) => text.setValue(this.plugin.settings.propReleaseDate).setDisabled(!this.plugin.settings.includeReleaseDate).onChange(async (value) => {
        this.plugin.settings.propReleaseDate = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Genres").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.includeGenres).onChange(async (value) => {
        this.plugin.settings.includeGenres = value;
        await this.plugin.saveSettings();
        this.display();
      })
    ).addText(
      (text) => text.setValue(this.plugin.settings.propGenres).setDisabled(!this.plugin.settings.includeGenres).onChange(async (value) => {
        this.plugin.settings.propGenres = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Backdrop").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.includeBackdrop).onChange(async (value) => {
        this.plugin.settings.includeBackdrop = value;
        await this.plugin.saveSettings();
        this.display();
      })
    ).addText(
      (text) => text.setValue(this.plugin.settings.propBackdrop).setDisabled(!this.plugin.settings.includeBackdrop).onChange(async (value) => {
        this.plugin.settings.propBackdrop = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Last episode watched").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.includeLastEpisodeWatched).onChange(async (value) => {
        this.plugin.settings.includeLastEpisodeWatched = value;
        await this.plugin.saveSettings();
        this.display();
      })
    ).addText(
      (text) => text.setValue(this.plugin.settings.propLastEpisodeWatched).setDisabled(!this.plugin.settings.includeLastEpisodeWatched).onChange(async (value) => {
        this.plugin.settings.propLastEpisodeWatched = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Last watched").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.includeLastWatched).onChange(async (value) => {
        this.plugin.settings.includeLastWatched = value;
        await this.plugin.saveSettings();
        this.display();
      })
    ).addText(
      (text) => text.setValue(this.plugin.settings.propLastWatched).setDisabled(this.plugin.settings.includeLastWatched === false).onChange(async (value) => {
        this.plugin.settings.propLastWatched = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Watched count").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.includeWatchedCount).onChange(async (value) => {
        this.plugin.settings.includeWatchedCount = value;
        await this.plugin.saveSettings();
        this.display();
      })
    ).addText(
      (text) => text.setValue(this.plugin.settings.propWatchedCount).setDisabled(this.plugin.settings.includeWatchedCount === false).onChange(async (value) => {
        this.plugin.settings.propWatchedCount = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Trakt ID").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.includeTraktId).onChange(async (value) => {
        this.plugin.settings.includeTraktId = value;
        await this.plugin.saveSettings();
        this.display();
      })
    ).addText(
      (text) => text.setValue(this.plugin.settings.propTraktId).setDisabled(!this.plugin.settings.includeTraktId).onChange(async (value) => {
        this.plugin.settings.propTraktId = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("TMDB ID").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.includeTmdbId).onChange(async (value) => {
        this.plugin.settings.includeTmdbId = value;
        await this.plugin.saveSettings();
        this.display();
      })
    ).addText(
      (text) => text.setValue(this.plugin.settings.propTmdbId).setDisabled(!this.plugin.settings.includeTmdbId).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Clear Trakt token").setDesc("Remove the saved Trakt token and re-authenticate on next sync.").addButton(
      (btn) => btn.setButtonText("Clear token").onClick(() => this.clearToken())
    );
    new import_obsidian.Setting(containerEl).setName("Reset to defaults").setDesc("Restore all plugin settings to their default values.").addButton(
      (btn) => btn.setButtonText("Reset").setWarning().onClick(async () => {
        this.plugin.settings = Object.assign({}, DEFAULT_SETTINGS);
        await this.plugin.saveSettings();
        new import_obsidian.Notice("Trakt Sync settings reset to defaults.");
        this.display();
      })
    );
  }
